...
def main():
    ...
    # 【新增：切片操作】只保留前 num_classes 个类别，去掉最后一位背景
    num_classes = len(dataset.metainfo['classes'])
    ...
    output_file_path = os.path.join(args.save_dir, 'PRF1.txt')
    with open(output_file_path, 'a') as output_file:
        # 如果你想写入平均值：
        output_file.write(f'AP: {average_precision:.5f}  AR: {average_recall:.5f}  F1: {np.mean(f1):.5f}\n')
        # 如果你想把每个类别的 Precision 都写进去，需要特殊处理（例如转为 string）
        # output_file.write(f'Per-class Precision: {np.array2string(precision, precision=5)}\n')
